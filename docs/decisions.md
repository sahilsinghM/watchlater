...
- **The whole pipeline must finish within the Vercel function's `maxDuration`.** On the **Hobby plan that is a hard 60s cap** (no override) — NOT the 300s an earlier draft of this doc assumed. If generation overruns, the function is killed mid-write and the job is stuck, surfacing as the processing-page timeout. So generation defaults to **Sonnet 4.6 with `max_tokens` bounded (~12k)** to finish well inside 60s, and the Anthropic SDK timeout (50s) aborts a slow run cleanly as `GENERATION_FAILURE` rather than letting the function be killed. To run heavier generation (Opus, larger budgets), first get headroom: raise `maxDuration` to 300 on Vercel Pro, or move generation to the `ingest-worker/` escape hatch (no 60s cap).
