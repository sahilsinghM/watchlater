        ...
        {segments.map((seg, i) => {
          ...
          return (
            ...
            >
              ...
              {/* Floating tooltip chip on hover. Lifted above the legend
                  (-top-[4.25rem]) and painted on an opaque card so it stays
                  readable instead of colliding with / bleeding through the
                  legend. z-20 keeps it over neighbouring segments. */}
              <span
                className={`pointer-events-none absolute -top-[4.25rem] left-1/2 z-20 -translate-x-1/2 whitespace-nowrap rounded-xl border-[3px] bg-card px-2.5 py-1 font-mono text-[10px] font-bold uppercase tracking-widest opacity-0 shadow-[3px_3px_0_0_var(--foreground)] transition-opacity duration-150 group-hover/seg:opacity-100 group-focus-visible/seg:opacity-100 ${s.tip}`}
              >
